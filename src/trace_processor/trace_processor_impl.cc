...
base::Status AbsTimeStr::Run(ClockTracker* tracker,
  ...
  base::Optional<int64_t> opt_abs_time = tracker->FromTraceTime(
  ...
  // There's no way to format with nanosecond precision in C so we need to
  // append
  char* str_res = static_cast<char*>(calloc(time_buf_size, sizeof(char)));
  snprintf(str_res, time_buf_size, "%s.%09" PRId64, timebuf,
           abs_time % nano_scale);
